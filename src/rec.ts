import { error, parseRec } from './tiny-ts-parser.ts';
...
const typeEqSub = (ty1: Type, ty2: Type, seen: [Type, Type][]): boolean => {
...
const simplifyType = (ty: Type): Type => {
  switch (ty.tag) {
    case 'Rec':
...
export const typecheck = (t: Term, tyEnv: TypeEnv): Type => {
  switch (t.tag) {
    ...
    default:
      throw new Error(`Unknown tag: ${(t as { tag: '__invalid__' }).tag}`);
  }
};
