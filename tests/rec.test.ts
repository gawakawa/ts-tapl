...
import { Term, Type, typecheck } from '../src/rec.ts';
...
Deno.test('typecheck recursive type with type variable', () => {
  // type Node = { value: number; next: Node };
  // const makeNode = (n: number): Node => { value: n, next: node };
  // makeNode(42)
  const recType: Type = {
    tag: 'Rec',
    name: 'Node',
    type: {
      tag: 'Object',
      props: [
        { name: 'value', type: { tag: 'Number' } },
        { name: 'next', type: { tag: 'TypeVar', name: 'Node' } },
      ],
    },
  };
  ...
  const term: Term = {
    ...
    init: {
      ...
      body: {
        tag: 'objectNew',
        props: [
          { name: 'value', term: { tag: 'var', name: 'n' } },
          { name: 'next', term: { tag: 'var', name: 'node' } },
        ],
      },
    },
    rest: {
      tag: 'call',
      func: { tag: 'var', name: 'makeNode' },
      args: [{ tag: 'number', n: 42 }],
    },
  };

  const result = typecheck(term, { node: recType });
  // Returns the expanded object type
  const expectedType: Type = {
    tag: 'Object',
    props: [
      { name: 'value', type: { tag: 'Number' } },
      { name: 'next', type: recType },
    ],
  };
  assertEquals(result, expectedType);
});

Deno.test('typecheck accessing property of recursive type', () => {
  ...
  const listType: Type = {
    ...
    type: {
      tag: 'Object',
      props: [
        { name: 'head', type: { tag: 'Number' } },
        { name: 'tail', type: { tag: 'TypeVar', name: 'List' } },
      ],
    },
  };
  ...
  const term: Term = {
    ...
    obj: {
      tag: 'objectGet',
      obj: { tag: 'var', name: 'list' },
      propName: 'tail',
    },
    propName: 'head',
  };

  const result = typecheck(term, { list: listType });
  assertEquals(result, { tag: 'Number' });
});

Deno.test('typecheck recursive type with function property', () => {
  // type Stream = { num: number; rest: () => Stream };
  // const makeStream = (n: number): Stream => { num: n, rest: () => stream };
  // makeStream(1)
      ...
      props: [
        { name: 'num', type: { tag: 'Number' } },
        {
          ...
          type: {
            tag: 'Func',
            params: [],
            retType: { tag: 'TypeVar', name: 'Stream' },
          },
        },
      ],
    },
  ...
  const term: Term = {
    ...
    init: {
      ...
      body: {
        tag: 'objectNew',
        props: [
          { name: 'num', term: { tag: 'var', name: 'n' } },
          {
            name: 'rest',
            term: {
              tag: 'func',
              params: [],
              body: { tag: 'var', name: 'stream' },
            },
          },
        ],
      },
    ...
    rest: {
      tag: 'call',
      func: { tag: 'var', name: 'makeStream' },
      args: [{ tag: 'number', n: 1 }],
    },
  };

  const result = typecheck(term, { stream: streamType });
  // Returns the expanded object type
  const expectedType: Type = {
    tag: 'Object',
    props: [
